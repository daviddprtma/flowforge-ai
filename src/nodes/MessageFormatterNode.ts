...
export type FormatterMedium = 'slack' | 'gmail' | 'gdocs';
...
function isMessageFormatterConfig(config: unknown): config is MessageFormatterConfig {
    ...
    return (
        ...
        (c.medium === 'slack' || c.medium === 'gmail' || c.medium === 'gdocs')
    );
}
...
export class MessageFormatterNode implements NodeExecutor {
    ...
    ): Promise<{ formattedText: string; medium: FormatterMedium; gmailLayout?: GmailLayout }> {
        if (!isMessageFormatterConfig(node.config)) {
            throw new Error(
                `Node "${node.id}" has an invalid or incomplete Message Formatter config. ` +
                `Expected: { medium: 'slack'| 'gmail' | 'gdocs', template: string }`,
            );
        }

        const { medium, template, gmailLayout = 'table' } = node.config;
