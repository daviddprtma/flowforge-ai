...
interface SlackConfig {
    credentialId: string;
    action: SlackAction;
    /** Who sends the message: 'bot' = FlowForge AI, 'user' = connected Slack account (default) */
    senderType?: 'bot' | 'user';
    /** FlowForge AI appearance overrides (only applied when senderType === 'bot') */
    botUsername?: string;    // display name override, e.g. "FlowForge AI"
...
export class SlackNode implements NodeExecutor {
    ...
    async execute(node: WorkflowNode, context: ExecutionContext): Promise<unknown> {
        ...
        // ── Resolve FlowForge AI appearance overrides (used in send actions) ─────
