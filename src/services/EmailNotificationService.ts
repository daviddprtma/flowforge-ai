...
function buildEmailText(p: ExecutionNotificationPayload, recipientTimeZone: string): string {
    ...
    const lines: string[] = [
        `FlowForge AI — Execution ${p.status.toUpperCase()}`,
        '='.repeat(50),
        '',
...
export class EmailNotificationService {
    ...
    async sendTestEmail(recipient: string): Promise<void> {
        ...
        const from = `"${process.env.SMTP_FROM_NAME ?? 'FlowForge AI'}" <${process.env.SMTP_FROM_ADDRESS}>`;

        const testHtml = buildFluxMessageHtml(
            'Test Email Successful',
            `<p style="font-size:15px;color:#374151;margin:0 0 16px;">
               Your <strong>FlowForge AI</strong> email notification settings are working correctly.
             </p>
        ...
        await transporter.sendMail({
            from,
            to: recipient,
            subject: '✅ FlowForge AI — Test Email',
            text: 'This is a test email from FlowForge AI. Your email notification settings are working correctly.',
            html: testHtml,
        });
    }
